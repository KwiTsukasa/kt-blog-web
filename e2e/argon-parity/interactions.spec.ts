...
function registerLocalInteractionSuite() {
  for (const interactionCase of ARGON_INTERACTION_CASES) {
    test(`local interaction surface exists: ${interactionCase.key}`, createInteractionSurfaceTest(interactionCase));
  }
  ...
  test('local blog modal delegates motion to antdv-next while keeping Blog theme tokens', createBlogModalThemeMotionSourceTest());
  test('local Blog animation and DOM contracts are centralized in factories', createBlogFactoryContractSourceTest());
  ...
  test('local post image placeholders match Argon lazyload geometry', createPostImagePlaceholderSurfaceTest());
  test('local post separators match live WordPress Argon surface', createPostSeparatorSurfaceTest());
  test('local header container matches live Argon navbar geometry', createHeaderContainerSurfaceTest());
  test('local sidebar menu items match live Argon menu surface and admin target', createSidebarMenuSurfaceTest());
  test('local sidebar overview keeps Argon tab-pane structure without archive navigation', createSidebarOverviewSurfaceTest());
  test('local post sidebar tabs switch catalog and overview panes', createPostSidebarTabTest());
  test('local post sidebar tabs fade between catalog and overview panes', createPostSidebarTabFadeTest());
  test('local post sidebar tab fade keeps Argon class-frame timing', createPostSidebarClassFrameSourceTest());
  test('local replaceable Argon DOM behavior avoids native id selectors', createReplaceableDomRefSourceTest());
  test('local sidebar search button matches Argon button interaction surface', createSidebarSearchButtonTest());
  test('local floating action buttons match Argon fixed surface and filter positioning', createFloatingActionSurfaceTest());
  test('local post share button matches Argon primary hover surface', createPostShareButtonSurfaceTest());
  test('local post navigation matches Argon card-body-free layout', createPostNavigationSurfaceTest());
  test('local rightbar archive links preserve month targets', createRightbarArchiveLinkTest());
  test('local article card link hover and target semantics match Argon', createArticleCardLinkInteractionTest());
}
...
async function prepareInteractionSurface(page: Page, interactionCase: ArgonInteractionCase) {
  ...
  if (interactionCase.key === '#fabtn_back_to_top' || interactionCase.key === '#fabtn_reading_progress') {
...
function createSettingsMutationTest() {
  ...
  return async function runSettingsMutationTest({ page }: PageFixture, testInfo: TestInfo) {
    ...
    await writeArgonArtifact(testInfo, 'settings-mutated', await captureAllRouteStates(page, homeRoute));
  };
}
...
function createSidebarSearchButtonTest() {
  ...
  return async function runSidebarSearchButtonTest({ page }: PageFixture) {
    ...
    const searchInput = page.locator('#leftbar_search_input input, input#leftbar_search_input').first();
...
function createPostShareButtonSurfaceTest() {
  ...
  return async function runPostShareButtonSurfaceTest({ page }: PageFixture) {
    ...
    await expect.poll(async () => shareButton.evaluate((element) => getComputedStyle(element).transform)).not.toBe('none');
  };
}
...
function createArticleCardLinkInteractionTest() {
  ...
  return async function runArticleCardLinkInteractionTest({ page }: PageFixture) {
    ...
    await title.hover();
    await expect.poll(async () => title.evaluate((element) => getComputedStyle(element).color)).not.toBe(titleColorBefore);
...
function createBlogModalThemeMotionSourceTest() {
  return async function runBlogModalThemeMotionSourceTest() {
    const modalSource = await readFile(new URL('../../src/components/blog/BlogModal.tsx', import.meta.url), 'utf8');
    const modalStyle = await readFile(new URL('../../src/styles/modal.scss', import.meta.url), 'utf8');
...
function createBlogFactoryContractSourceTest() {
  return async function runBlogFactoryContractSourceTest() {
    const animationFactory = await readFile(new URL('../../src/factories/blogAnimationFactory.ts', import.meta.url), 'utf8');
    const domFactory = await readFile(new URL('../../src/factories/blogDomFactory.ts', import.meta.url), 'utf8');
    const sidebarSource = await readFile(new URL('../../src/components/blog/BlogSidebar.tsx', import.meta.url), 'utf8');
    const layoutSource = await readFile(new URL('../../src/components/blog/BlogLayout.tsx', import.meta.url), 'utf8');
    const floatActionsSource = await readFile(new URL('../../src/components/blog/BlogFloatActions.tsx', import.meta.url), 'utf8');
    const shareSource = await readFile(new URL('../../src/components/blog/BlogShare.tsx', import.meta.url), 'utf8');
    const postPageSource = await readFile(new URL('../../src/views/blog/PostPage.tsx', import.meta.url), 'utf8');
    const argonEffectsSource = await readFile(new URL('../../src/hooks/useArgonEffects.ts', import.meta.url), 'utf8');
    const domRefsSource = await readFile(new URL('../../src/hooks/useBlogDomRefs.ts', import.meta.url), 'utf8');
    ...
    expect(sidebarSource).not.toMatch(/const ARGON_CATALOG_(?:MANUAL_LOCK_MS|OFFSET|SCROLL_EXTRA|NORMAL_ANIMATION_MS)\s*=/);
    expect(sidebarSource).not.toContain('window.requestAnimationFrame');
    expect(sidebarSource).not.toContain('setTimeout(() =>');
    expect(layoutSource).not.toMatch(/id="(?:banner|banner_container|content|sidebar_mask|primary|main|footer)"/);
    expect(floatActionsSource).not.toMatch(/id="(?:fabtn_toggle_sides|fabtn_back_to_top|fabtn_toggle_blog_settings_popup|fabtn_reading_progress|blog_settings_popup)"/);
    expect(floatActionsSource).not.toContain('}, 300)');
    expect(shareSource).not.toMatch(/id="(?:share_container|share_show|share)"/);
    expect(postPageSource).not.toMatch(/id="(?:post_content|comments|post_comment|post_comment_content|post_comment_send)"/);
...
function createTaxonomyModalTest() {
  ...
  return async function runTaxonomyModalTest({ page }: PageFixture) {
    ...
    await expect.poll(() => categoriesPanel.evaluate((element) => getComputedStyle(element).transform)).toBe('none');
    const categoriesPanelBox = await categoriesPanel.boundingBox();
    const expectedCenteredTop = (page.viewportSize()!.height - (categoriesPanelBox?.height ?? 0)) / 2;
    ...
    const categoryChips = await categoriesModal.locator('.kt-blog__tag').evaluateAll((items) =>
    ...
    const tagLabels = await tagsModal.locator('.kt-blog__tag').evaluateAll((items) =>
...
function createPostInteractionTest() {
  ...
  return async function runPostInteractionTest({ page }: PageFixture, testInfo: TestInfo) {
    ...
    await page.locator('#post_comment_content, .kt-blog__comment-textarea').first().fill('Argon parity smoke');
    ...
    await expect(page.locator('#comment_emotion_btn')).toHaveCSS('background-color', 'rgba(0, 0, 0, 0)');
    ...
    await writeArgonArtifact(testInfo, 'post-50-interactions', await captureAllRouteStates(page, postRoute));
  };
}
...
function createPostInlineLinkSurfaceTest() {
  ...
  return async function runPostInlineLinkSurfaceTest({ page }: PageFixture) {
    ...
    expect(
      await link.evaluate((element) => getComputedStyle(element, '::before').transform),
...
function createPostCodeblockSurfaceTest() {
  ...
  return async function runPostCodeblockSurfaceTest({ page }: PageFixture) {
    ...
    expect(await readPseudoStyle(pre, '::before', 'backgroundImage')).toContain('data:image/svg+xml');
...
function createSidebarOverviewSurfaceTest() {
  ...
  return async function runSidebarOverviewSurfaceTest({ page }: PageFixture) {
    ...
    await expect(page.locator('#leftbar_part2_inner > .kt-blog__sidebar-overview-content')).toHaveCount(0);
...
function createPostSidebarTabTest() {
  ...
  return async function runPostSidebarTabTest({ page }: PageFixture) {
    ...
    const tabLabels = await page.locator('.kt-blog__sidebar-tabs [id^="leftbar_tab_"]').evaluateAll((tabs) =>
    ...
    await expect(page.locator('#leftbar_tab_catalog')).toHaveCSS('transition', 'opacity 0.15s linear');
    await expect(page.locator('#leftbar_tab_overview')).toHaveCSS('transition', 'opacity 0.15s linear');
    ...
    await expect(page.locator('#leftbar_tab_catalog_btn')).toHaveClass(/kt-blog__sidebar-tab--active/);
    ...
    await expect(page.locator('#leftbar_catalog .index-link').first()).toHaveAttribute('href', '#header-id-1');
    await expect(page.locator('#leftbar_catalog .index-subItem-box').first()).toHaveCSS('display', 'none');
    await expect(page.locator('#leftbar_catalog .index-link').first()).toHaveCSS('font-size', '15px');
    await expect(page.locator('#leftbar_catalog .index-link').first()).toHaveCSS('padding', '4px 8px');
    await expect(page.locator('#leftbar_catalog .index-link').first()).toHaveCSS('border-left-width', '3px');
    ...
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('background-color', 'rgb(60, 54, 67)');
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('border-radius', '3px');
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('font-size', '14px');
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('line-height', '19.25px');
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('padding', '14px');
    await expect(page.locator('#post_content .wp-block-preformatted').first()).toHaveCSS('border-top-width', '1px');
    ...
    await expect(page.locator('#post_content p code').first()).toHaveCSS('background-color', 'rgb(60, 54, 67)');
    ...
    await expect(page.locator('#post_content figure.wp-block-table').first()).toHaveCSS('margin', '48px 0px');
    await expect(page.locator('#post_content figure.wp-block-table').first()).toHaveCSS('overflow-x', 'auto');
    await expect(page.locator('#post_content figure.wp-block-table table').first()).toHaveCSS('display', 'table');
    await expect(page.locator('#post_content figure.wp-block-table table').first()).toHaveCSS('table-layout', 'fixed');
    await expect(page.locator('#post_content figure.wp-block-table table').first()).toHaveCSS('white-space', 'normal');
    await expect(page.locator('#post_content figure.wp-block-table table').first()).toHaveCSS('word-break', 'break-word');
    await expect(page.locator('#post_content figure.wp-block-table th').first()).toHaveCSS('padding', '8px');
    await expect(page.locator('#post_content figure.wp-block-table th').first()).toHaveCSS('text-align', 'left');
    await expect(page.locator('#post_content figure.wp-block-table th').first()).toHaveCSS('vertical-align', 'top');
    await expect(page.locator('#post_content figure.wp-block-table td').first()).toHaveCSS('padding', '8px');
    await expect(page.locator('#post_content figure.wp-block-table td').first()).toHaveCSS('vertical-align', 'top');
    const postContentStats = await page.locator('#post_content').evaluate((element) => {
      ...
      return {
        header2OffsetTop: Math.round((secondHeading?.getBoundingClientRect().top ?? 0) + window.scrollY),
    ...
    await page.evaluate((scrollTop) => window.scrollTo(0, scrollTop - 1), secondHeadingActiveBoundary);
    await expect
      .poll(
        () => page.locator('#leftbar_catalog .index-item.current > .index-link').evaluate((element) => element.textContent?.trim()),
    ...
    await expect(page.locator('#leftbar_catalog .index-item.current > .index-link')).toHaveText('2. 建设目标');
    await expect(page.locator('#leftbar_catalog .index-subItem-box').first()).toHaveCSS('display', 'block');
    ...
    await expect
      ...
      .toBe(await page.locator('#leftbar_catalog .index-item.current > .index-link').evaluate(() => {
        const probe = document.createElement('span');
        probe.style.color = document.documentElement.classList.contains('darkmode')
          ? 'var(--themecolor-light)'
          : 'var(--themecolor)';
    ...
    await expect
      .poll(() => page.evaluate(() => window.scrollY), { message: 'catalog click scrolls the document' })
      .toBeGreaterThan(beforeClickScrollY + 850);
    await expect
      .poll(() => page.locator('#header-id-2').evaluate((element) => Math.round(element.getBoundingClientRect().top)))
      .toBeLessThan(120);
    await page.waitForTimeout(450);
    await page.evaluate(() => window.scrollTo(0, Math.round(document.documentElement.scrollHeight * 0.75)));
    await expect
      .poll(
        () => page.locator('#leftbar_catalog .index-item.current > .index-link').evaluate((element) => element.textContent?.trim()),
    ...
    await expect(page.locator('#leftbar_tab_overview_btn')).toHaveClass(/kt-blog__sidebar-tab--active/);
    ...
    await expect(page.locator('.kt-blog__site-stats-item').nth(1)).toHaveCSS('border-left-width', '1px');
...
function createPostSidebarClassFrameSourceTest() {
  return async function runPostSidebarClassFrameSourceTest() {
    const source = await readFile(new URL('../../src/components/blog/BlogSidebar.tsx', import.meta.url), 'utf8');
...
function createReplaceableDomRefSourceTest() {
  return async function runReplaceableDomRefSourceTest() {
    const sidebarSource = await readFile(new URL('../../src/components/blog/BlogSidebar.tsx', import.meta.url), 'utf8');
    const postPageSource = await readFile(new URL('../../src/views/blog/PostPage.tsx', import.meta.url), 'utf8');
    const commentsStyle = await readFile(new URL('../../src/styles/searchComments.scss', import.meta.url), 'utf8');
    ...
    expect(`${sidebarSource}\n${postPageSource}`).not.toMatch(/document\.(?:getElementById|querySelector(?:All)?)\([^)]*['"]#?(?:leftbar|post_content|post_comment)/);
...
function createPostSidebarTabFadeTest() {
  ...
  return async function runPostSidebarTabFadeTest({ page }: PageFixture) {
    ...
    const switchingOpacity = await overviewPane.evaluate((element) => Number(getComputedStyle(element).opacity));
