...
/**
 * Interpolates one transform scalar using the exact legacy branch ordering.
 * @param samples Authored transform samples selected by the interpolation corner table.
 * @param cornerIndexes Resolved source indexes for the active interpolation hypercube.
...
function interpolateTransformField(
  ...
  const cornerCount = Math.pow(2, gridDimensionCount) | 0
  const highDimensionalCornerWeights = new Float32Array(cornerCount)
  for (let cornerIndex = 0; cornerIndex < cornerCount; cornerIndex += 1) {
    let bitCursor = cornerIndex
    let combinedWeight = 1
    for (let axisIndex = 0; axisIndex < gridDimensionCount; axisIndex += 1) {
      combinedWeight *= bitCursor % 2 === 0 ? 1 - cornerWeights[axisIndex]! : cornerWeights[axisIndex]!
      bitCursor /= 2
    }
  ...
  for (let cornerIndex = 0; cornerIndex < cornerCount; cornerIndex += 1) {
    interpolatedValue +=
      highDimensionalCornerWeights[cornerIndex]! *
      readTransformField(samples, cornerIndexes, cornerIndex, field)
  }
 ...
 * @param options Base-data, context, transform-value, math, and debug dependencies from the compatibility capsule.
...
export function createCubism2TransformBaseData(
  ...
  TransformBaseData.prototype.applyRuntimeContext = function (
    ...
    const targetBaseContext = runtimeModelContext.getBaseContext(runtimeContext.targetBaseDataIndex)
    if (targetBaseContext == null) {
      runtimeContext.setActive(false)
      return
    }
