...
export interface Cubism2BaseDataPayload {
  baseDataId: unknown | null
  targetBaseDataId: unknown | null
}

export interface Cubism2BaseDataOpacityPayload {
...
/**
 * Reads the shared base-data ID payload in the exact min.js object order.
 * @param reader Binary reader positioned at the shared base-data payload.
 * @returns Named base-data payload carrying the base ID and target base ID.
 */
function readCubism2BaseDataPayload(reader: Cubism2BaseDataReader): Cubism2BaseDataPayload {
  const baseDataId = reader.readObject()
  const targetBaseDataId = reader.readObject()
...
/**
 * Applies a decoded base-data ID payload to the shared base-data instance.
 * @param baseData Base-data object that owns transform/grid dependency IDs.
 * @param payload Payload returned by `readCubism2BaseDataPayload`.
 */
function applyCubism2BaseDataPayload(
  baseData: Cubism2BaseDataInstance,
  payload: Cubism2BaseDataPayload,
): void {
  baseData.baseDataId = payload.baseDataId
  baseData.targetBaseDataId = payload.targetBaseDataId
}

/**
 * Reads the optional SDK2 v2.10 opacity payload behind the min.js format-version gate.
 * @param reader Binary reader positioned at the optional v2 opacity payload.
 * @param minimumFormatVersion First MOC format version that stores opacity timelines.
 * @returns Named opacity payload; older payloads keep `opacityValues` null.
 */
function readCubism2BaseDataOpacityPayload(
  reader: Cubism2BaseDataReader,
  minimumFormatVersion: number,
): Cubism2BaseDataOpacityPayload {
  if (reader.getFormatVersion() >= minimumFormatVersion) {
    return {
      opacityValues: reader.readFloat32Array(),
    }
  }
...
/**
 * Applies decoded opacity values when the optional v2.10 payload is present.
 * @param baseData Base-data object that owns the opacity timeline.
 * @param payload Payload returned by `readCubism2BaseDataOpacityPayload`.
 */
function applyCubism2BaseDataOpacityPayload(
  baseData: Cubism2BaseDataInstance,
  payload: Cubism2BaseDataOpacityPayload,
): void {
  if (payload.opacityValues != null) {
    baseData.opacityValues = payload.opacityValues
  }
}
 ...
 * @returns Base-data constructor bound to the supplied compatibility dependencies.
 */
export function createCubism2BaseData(
  ...
  BaseData.prototype.readBaseData = function (reader: Cubism2BaseDataReader): void {
    applyCubism2BaseDataPayload(this, readCubism2BaseDataPayload(reader))
  }
  ...
  BaseData.prototype.readV2Opacity = function (reader: Cubism2BaseDataReader): void {
    applyCubism2BaseDataOpacityPayload(
      this,
      readCubism2BaseDataOpacityPayload(
        reader,
