...
export interface Cubism2AMotionInstance {
  ...
  updateParam: (model: unknown, motionQueueEntry: Cubism2MotionQueueEntryInstance) => void
  updateParamExe: (
    model: unknown,
...
export interface Cubism2MotionQueueManagerInstance {
  ...
  stopAllMotions: () => void
  updateParam: (model: unknown) => boolean
...
export function createCubism2MotionBase(
   ...
   */
  function calculateLegacyCurveWeight(
    elapsedMillis: number,
    durationMillis: number,
    curvePositionMillis: number,
  ): number {
  ...
  BaseMotion.calculateLegacyCurveWeight = calculateLegacyCurveWeight
   ...
   */
  QueueManager.prototype.updateParam = function (model: unknown): boolean {
    try {
  ...
  MotionQueueEntry.prototype.scheduleFadeOut = scheduleFadeOut
