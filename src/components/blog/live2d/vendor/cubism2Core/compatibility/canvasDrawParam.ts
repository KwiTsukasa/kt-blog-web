...
export type Cubism2CanvasFloatBuffer = Cubism2LegacyWritableFloatBuffer
export type Cubism2CanvasIndexBuffer = Cubism2LegacyWritableIndexBuffer
 ...
 * @param options Runtime dependencies from the Cubism2 compatibility capsule.
...
export function createCubism2CanvasDrawParam(
   ...
   * @returns Float buffer cast to the legacy writable-buffer interface.
   */
   ...
   * @returns Index buffer cast to the legacy writable-buffer interface.
   */
  ...
  /**
   * Ensures a Float32 staging buffer can hold the supplied values and rewinds it.
   * @param buffer Existing buffer, if any.
   * @param values Values copied into the buffer.
   * @returns Writable float buffer positioned at zero.
   */
  ...
  /**
   * Ensures an Int16 staging buffer can hold the supplied triangle indexes and rewinds it.
   * @param buffer Existing buffer, if any.
   * @param values Triangle index values copied into the buffer.
   * @returns Writable index buffer positioned at zero.
   */
  ...
  CanvasDrawParam.prototype.drawTexture = function (
    ...
    const expandedStrokeWidth = opacity > 0.9 ? options.Live2D.EXPAND_W : 0
   ...
   */
  CanvasDrawParam.prototype.setDrawParam = function (drawParam: unknown): never {
    void drawParam
