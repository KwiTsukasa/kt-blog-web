...
import BlogLive2DPickerModal from '../dialogs/BlogLive2DPickerModal'
import { BLOG_LIVE2D_MODELS } from './runtime/live2dRuntimeCatalog'
import type {
  Live2DModelSettings,
  Live2DResolvedState,
  Live2DTSRuntime,
} from './runtime/live2dRuntimeTypes'
import { createLive2DTSRuntime } from './runtime/live2dTsRuntime'
import { WORDPRESS_WAIFU_MIN_WIDTH } from './wordpressWidgetConfig'
import {
  mountWordPressWidgetController,
  type WordPressWidgetControllerHandle,
} from './wordpressWidgetController'
import { WORDPRESS_WAIFU_TOOLS } from './wordpressWidgetConfig'
...
export default defineComponent({
  ...
  setup() {
    ...
    const resolveControllerElements = () => {
      ...
      const sendButton = sendButtonRef.value
      const tips = tipsRef.value
      const tool = toolRef.value
      const widget = widgetRef.value
      if (!canvas || !chat) return null
      if (!closeButton || !input) return null
      if (!sendButton || !tips) return null
        ...
        <>
          <div ref={widgetRef} class="waifu kt-blog__live2d-widget">
            ...
            <canvas
              ref={canvasRef}
              id="live2d"
              class="live2d kt-blog__live2d-canvas"
              height={250}
              width={280}
            />
...
/**
 * 将纹理索引越界错误转为服装不存在提示，其他失败统一返回 Live2D 重试提示。
 * @param error - 待转换、记录或继续传播的异常。
 * @returns 可直接展示的服装不存在或 Live2D 重试提示。
 */
