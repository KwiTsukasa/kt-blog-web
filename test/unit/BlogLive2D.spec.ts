...
vi.mock('@/components/blog/dialogs/BlogModal', async () => {
        ...
        return () =>
          props.open
            ? vue.h('section', { class: ['mock-blog-modal', props.className] }, [
                ...
                vue.h(
                  'button',
                  { class: 'mock-blog-modal__close', onClick: () => emit('close') },
                  '关闭',
                ),
                slots.default?.(),
                slots.footer
                  ? vue.h('footer', { class: 'mock-blog-modal__footer' }, slots.footer())
                  : null,
              ])
            : null
      },
...
describe('BlogLive2D', () => {
  ...
  it('mounts the TypeScript runtime without appending the old WordPress script', async () => {
    ...
    expect(appendedScripts.some((src) => src.includes('/live2d/wordpress-moc/live2d.min.js'))).toBe(
      false,
    )
  })
  ...
  it('keeps the model picker open and reports a stable error when switching fails', async () => {
    ...
    document.querySelector<HTMLElement>('.waifu-tool .fui-user')?.click()
    await flushPromises()
    const tiaButton = Array.from(
      document.querySelectorAll<HTMLButtonElement>('.kt-blog__live2d-picker-option'),
    ).find((button) => button.textContent?.includes('Tia'))
    tiaButton?.click()
    await flushPromises()
  ...
  it('previews a Chinese-named costume before committing it through the direct runtime API', async () => {
    ...
    const secondTextureButton = buttons.find((button) =>
      button.textContent?.includes('天蓝色比基尼'),
    )
    ...
    expect(
      document.querySelector('.mock-blog-modal.kt-blog__live2d-picker-modal')?.textContent,
    ).toContain('正在预览：天蓝色比基尼')
    const confirmButton = Array.from(document.querySelectorAll<HTMLButtonElement>('button')).find(
      (button) => button.textContent?.includes('使用此服装'),
    )
    confirmButton?.click()
    await flushPromises()
  ...
  it('keeps the texture picker open when the runtime rejects a costume preview', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => undefined)
    runtimeMocks.runtime.previewTexture.mockRejectedValueOnce(
      new Error('Live2D texture index is out of range.'),
    )
    const wrapper = await mountDesktopLive2D()
